...
```{r}
gdp_long <- gdp |>
  filter(!str_detect(TIME...1, "^GEO"), TIME...2 != "GEO (Labels)") |>
...
# 1) Finn år-kolonner
year_cols <- intersect(names(Pop), as.character(2000:2023))

# 2) Rydd bort GEO-rader (hvis ikke gjort)
Pop <- Pop %>%
  filter(!str_detect(geo_kode, "^GEO (Codes)"), geo_navn != "GEO (Labels)")

# 3) Gjør ALLE år-kolonner til numeric, robust for både character og double
Pop <- Pop %>%
  mutate(
    across(
      all_of(year_cols),
      ~ readr::parse_number(
          as.character(.),                     # tving til tekst først
          locale = readr::locale(
            decimal_mark   = ",",              # 23 859,75
            grouping_mark  = " "               # tusenskilletegn
          )
        )
    )
  )
...
```{r}
library(dplyr)
library(readr)  # for parse_number if du trenger det
...
# Valgfritt: 2023-oversikt for dine land (ES, AT, CZ, SI, EE)
...
stats_land_all <- gdpc %>%
  ...
  summarise(
    ...
    q1      = quantile(gdpc_eur, 0.25, na.rm = TRUE),
    median  = median(gdpc_eur, na.rm = TRUE),
    q3      = quantile(gdpc_eur, 0.75, na.rm = TRUE),
...
Artikkelen bygger et globalt datasett for regional inntektsulikhet ved å predikere regionale inntekter fra nattlys-satellittdata, og bruker disse til å beregne standard mål som befolkningsvektet Gini og generaliserte entropi-mål. Poenget er at lysintensitet henger stabilt sammen med regional økonomisk aktivitet, og at predikert inntekt (korrigert for målefeil som «top-coding» og «zero-coding») fanger reelle (ikke bare nominelle) forskjeller bedre enn lys alene.
...
# 1) Kvalitetsfilter (valgfritt, men lurt):
...
# 3) Lag også 2023-snitt og ev. rangeringer (nyttig til tabeller/figurer)
